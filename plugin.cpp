...
inline void replace_or_delete(cexpr_t *expr, cexpr_t *replacement,
                              bool success) {
  if (!replacement)
    return;
...
inline void merge_accesses(cexpr_t *&original, cexpr_t *access, ctype_t op,
                           ea_t ea, tinfo_t type) {
  if (!access)
    return;
...
// selects (adds memref expr) for the first member that is a struct inside of an
// union
...
#if IDA_SDK_VERSION > 830
inline cexpr_t *create_bitfield_access(access_info &info, udm_t &member,
                                       ea_t original_ea, tinfo_t &common_type)
#elif IDA_SDK_VERSION <= 830

inline cexpr_t *create_bitfield_access(access_info &info, udt_member_t &member,
                                       ea_t original_ea, tinfo_t &common_type)
#endif
{
...
// executes callback for each member in `type` where its offset coincides with
// `and_mask`. `cmp_mask` is used to calculate enabled bits in the bitfield.
template <class Callback>
bool for_each_bitfield(Callback cb, tinfo_t type, uint64_t and_mask) {
  ...
  for (size_t i = 0; i < 64; ++i) {
...
#if IDA_SDK_VERSION <= 830
    if (type.find_udt_member(&member, STRMEM_INDEX) == -1)
      continue;
#elif IDA_SDK_VERSION > 850
    if (type.find_udm(&member, STRMEM_INDEX) == -1)
      continue;
#endif
    ...
    if (member.size != 1 && (and_mask & mask) != mask) {
      msg("[bitfields] bad offset (%ull) and size (%ull) combo of a field for "
          "given mask (%ull)\n",
...
inline access_info unwrap_access(cexpr_t *expr) {
  ...
  if (expr->op == cot_band) {
    ...
    constexpr static std::tuple<std::string_view, uint64_t, uint8_t>
        functions[] = {
    ...
    auto it = std::ranges::find(functions, expr->x->helper,
                                [](auto &&func) { return std::get<0>(func); });
  ...
  if (expr->op != cot_ptr || expr->x->op != cot_cast ||
      expr->x->x->op != cot_ref)
    return res;
  ...
  if (use_ea == BADADDR)
    msg("[bitfields] can't find parent ea - won't be able to save union "
        "selection\n");
...
inline void handle_comparison(cexpr_t *expr) {
  ...
  auto success = for_each_bitfield(
...
#endif
      {
        // construct the call / access itself
        auto access =
            create_bitfield_access(info, member, expr->ea, eq_num->type);
        if (!access)
          return;

        const auto mask = bitfield_access_mask(member);
        const auto value =
            ((eq_num->n->_value << info.shift_value) & mask) >> member.offset;

        // if the flag is multi byte, reconstruct the comparison
        if (member.size > 1) {
          auto num = new cnumber_t();
          num->assign(value, access->type.get_size(),
                      member.type.is_signed() ? type_signed : type_unsigned);
        ...
        // otherwise the flag is single bit; if the flag is disabled, add
        // logical not
        ...
        merge_accesses(replacement, access,
                       expr->op == cot_eq ? cot_land : cot_lor, expr->ea,
                       tinfo_t{BTF_BOOL});
      },
...
#endif
      {
        // TODO: for assignment where more than 1 field is being accessed create
        // a new bitfield type for the result that would contain the correctly
        // masked and shifted fields
        const auto access =
            create_bitfield_access(info, member, expr->y->ea, expr->x->type);
...
inline void handle_or_assignment(cexpr_t *expr) {
  ...
  cexpr_t *arg0 = expr->x;
  if (arg0->op != cot_ptr || arg0->x->op != cot_cast ||
      arg0->x->x->op != cot_ref)
    return;

  // these functions will reference the union directly, so select a field for a
  // start
...
inline void handle_call(cexpr_t *expr) {
  ...
  constexpr static std::string_view functions[] = {
      // bit mask functions
      "_InterlockedOr8", "_InterlockedOr16", "_InterlockedOr",
      "_InterlockedOr64", "_InterlockedAnd8", "_InterlockedAnd16",
      "_InterlockedAnd", "_InterlockedAnd64",
      // bit index functions
      "_bittest", "_bittest64", "_bittestandreset", "_bittestandreset64",
      "_bittestandset", "_bittestandset64", "_interlockedbittestandset",
...
struct bitfields : plugmod_t {
  ...
  void set_state(bool s) { bitfields_optimizer.set_state(s); }
  bitfields() { set_state(nn.altval(0) == 0); }
  ~bitfields() { bitfields_optimizer.uninstall(); }
...
State: %s)";
    int code = ask_buttons("~E~nable", "~D~isable", "~C~lose", -1, format + 1,
